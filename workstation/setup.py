...
def run_setup() -> int:
    ...
    _print("  MCP 工具及用途:")
    _print("    list_dir     摸清工程结构、找源码和 hex/bin 产物")
    _print("    read_file    看源码、日志、map；二进制用 encoding=hex")
    _print("    write_file   改源码、生成编译/烧录脚本")
    _print("    run_command  编译、烧录、跑测试（走系统 PATH）")
    _print("    serial_list  列出 COM 口，确认板子插在哪")
    _print("    serial_send  串口日志 / AT / bootloader")
    _print("    take_photo   拍板子，核对 LED/屏幕等现象")
    _print()
